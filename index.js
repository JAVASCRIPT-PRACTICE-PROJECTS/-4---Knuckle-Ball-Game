...
// permits ball to move with space key
...
function increaseScore(){
    // increase counter when ball passes pipe
...
function collisionCheck(){
    ...
    // check collision from upper box
    if(
    ...
    // verify if ball has hit boundaries
...
function loop(){
    ...
    // add collision to display end menu and end the game
    if(collisionCheck()){
        endGame();
        return;
    }
    ...
    pipeX -=  1.5;
    // reset game when pipe moves out of frame
    if(pipeX < -50){
        pipeX = 400;
